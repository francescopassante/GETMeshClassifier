...
# Let's check the implementation with the W_basis computed from W_basis.py
# AssumiGEUtilsumpy arrays, we need to convert them to torch tensors
...
print(W_basis_torch[0])
# Now we can create an instance of the LinearEquivariant layer
equivariant_layer = LinearEquivariant(W_basis=W_basis_torch, num_fields=12)
# Example input: batch of 2 samples, each with 5 points, and 3 features (x, y, z)
input_features = torch.randn(2, 5, 3)
output_features = equivariant_layer(input_features)
print(output_features.shape)  # Should be (2, 5, 108)
